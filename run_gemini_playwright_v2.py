...
def clean_semantic_block(content):
    ...
    # 1. Strip backslashes from common escaped characters (comprehensive set)
    # NOTE: Double-quote (") is deliberately EXCLUDED — \" is legitimate JSON
    # string escaping that must be preserved for the JSON parser.
...
def heuristic_extract_blocks(text):
    ...
    mappings = {
        ...
        'GPF-PROBLEM-DEFINITION': [r'GPF-PROBLEM-DEFINITION', r'Core Problem Definition', r'Problem Definition'],
        'GPF-ABSTRACTION-LEVEL': [r'GPF-ABSTRACTION-LEVEL', r'Abstraction Level'],
        'GPF-ARCHITECTURE': [r'GPF-ARCHITECTURE', r'Problem Architecture', r'Architecture'],
        'GPF-FORMAL-SPECIFICATION': [r'GPF-FORMAL-SPECIFICATION', r'Formal Specification'],
        'GPF-EVALUATION': [r'GPF-EVALUATION', r'Evaluation & Verification', r'Evaluation'],
        'GPF-CONSTRAINTS': [r'GPF-CONSTRAINTS', r'Constraints & Context', r'Constraints'],
        'GPF-CLASSIFICATION': [r'GPF-CLASSIFICATION', r'Problem Classification', r'Classification'],
        'GPF-RELATIONAL-NETWORK': [r'GPF-RELATIONAL-NETWORK', r'Relational Network'],
        'GPF-EXEMPLARS': [r'GPF-EXEMPLARS', r'Exemplars'],
        'GPF-HISTORY': [r'GPF-HISTORY', r'History & Lineage', r'History'],
    }
    ...
    for chunk in chunks:
        ...
        for block_key, patterns in mappings.items():
            for p in patterns:
                if re.search(rf'^[#\s\*\\!]*?(?:\d+\.\s*)?{p}', chunk, re.IGNORECASE):
                    current_block = block_key
                    found_header = True
                    # Strip the header portion
                    content = re.sub(rf'^[#\s\*\\!]*?(?:\d+\.\s*)?{p}[#\s\*:\\]*', '', chunk, flags=re.IGNORECASE).strip()
...
You MUST output your response using granular blocks delimited by !!!!!BLOCK-NAME!!!!! tags (e.g., !!!!!METADATA!!!!!, !!!!!REASONING!!!!!, !!!!!GPF-ARCHITECTURE!!!!!, !!!!!TURN-3-USER!!!!!).
DO NOT use markdown fenced code blocks (like ```json or ```) ANYWHERE in your response. Fenced blocks trigger Canvas mode and crash the agent. Write all code and JSON as raw plain-text indented by 4 spaces. The METADATA block must use raw JSON.
IMPORTANT: Prioritize COMPLETING the entire 17-block structure over adding more detail. A truncated response is worse than a slightly shorter but complete one.
...
"""
    ...
    with sync_playwright() as p:
        ...
        # If the pipeline was Ctrl+C'd, the previous browser.close() never ran,
        # leaving zombie Chromium processes that hold a lock on the profile dir.
        # This causes the next launch to hang on about:blank indefinitely.
        try:
            import psutil
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    if 'chrom' not in pname:
                        continue
                    cmdline = proc.info.get('cmdline') or []
                    cmd_str = ' '.join(cmdline)
                    if profile_abs.replace('/', '\\') in cmd_str or profile_abs.replace('\\', '/') in cmd_str:
                        proc.kill()
                        killed += 1
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
            if killed:
                log(f"Killed {killed} orphan Chromium process(es) using our profile")
                time.sleep(2)  # Give OS time to release file locks
        except ImportError:
            # psutil not installed — try basic taskkill as fallback
            try:
                subprocess.run('taskkill /F /IM chromium.exe /T 2>NUL', shell=True,
        ...
        # Also clear the "Tabs" state and lock files that cause restore prompts
        ...
        log("Cleared session restore files and lock files")
        ...
        # ── CLOSE STALE TABS FROM PERSISTENT CONTEXT ─────────────────────
        # When using a persistent profile, Chrome may restore old tabs (about:blank,
        # previous Gemini sessions, etc.) which compete for resources and cause hangs.
        # IMPORTANT: A persistent context MUST always have at least one page open,
        # so we reuse the first existing tab and only close the extras.
        existing_pages = list(browser.pages)  # snapshot
        if existing_pages:
            page = existing_pages[0]  # reuse the first tab
            # Close all other stale tabs
            for stale in existing_pages[1:]:
                try:
                    stale.close()
                except Exception:
                    pass
            if len(existing_pages) > 1:
                log(f"Closed {len(existing_pages) - 1} extra stale tab(s) from persistent profile")
        else:
            page = browser.new_page()

        # ── BLOCK CANVAS/IMMERSIVE MODE VIA ROUTE INTERCEPTION ──────────────
        # Canvas is triggered by iframe navigation to goog*.usercontent.com/immersive_entry_chip/*
        # Blocking this URL at the network level prevents Canvas from ever loading.
        def _block_canvas_route(route):
            log(f"  [Canvas] BLOCKED Canvas request: {route.request.url[:100]}")
            route.abort()

        page.route("**/*usercontent.com*immersive*", _block_canvas_route)
        page.route("**/*immersive_entry_chip*", _block_canvas_route)
        page.route("**/*usercontent.com*canvas*", _block_canvas_route)
        log("Canvas route interception installed (blocking usercontent.com/immersive_entry_chip)")

        # Close any Canvas popup tabs that Gemini might open
        def _on_new_page(new_page):
            try:
                url = new_page.url or ""
                if 'usercontent.com' in url or 'immersive' in url:
                    log(f"  [Canvas] Closed Canvas popup tab: {url[:80]}")
                    new_page.close()
            except Exception:
                pass
        browser.on("page", _on_new_page)

        # ── NAVIGATE TO GEMINI WITH RETRY ────────────────────────────────
        # Use a 60s timeout (up from default 30s) and retry with a fresh page
        # if the first attempt fails (e.g. due to slow profile initialization).
        nav_success = False
        for nav_attempt in range(3):
            try:
                ...
                log(f"  ⚠️ Navigation attempt {nav_attempt+1}/3 failed: {nav_err}")
                if nav_attempt < 2:
                    # Close this page and try a fresh one
                    try:
                        page.close()
                    except Exception:
                        pass
                    page = browser.new_page()
                    # Re-install canvas blockers on new page
                    page.route("**/*usercontent.com*immersive*", _block_canvas_route)
                    page.route("**/*immersive_entry_chip*", _block_canvas_route)
                    page.route("**/*usercontent.com*canvas*", _block_canvas_route)
                    time.sleep(2)
        ...
        if not nav_success:
            log("FATAL: Could not navigate to Gemini after 3 attempts")
            browser.close()
            return False
        ...
        # ── INJECT MUTATION OBSERVER: Auto-remove Canvas iframes from DOM ──
        try:
            page.evaluate("""() => {
                if (window.__canvasBlockerActive) return;
                const observer = new MutationObserver((mutations) => {
                    for (const m of mutations) {
                        for (const node of m.addedNodes) {
                            if (node.nodeType !== 1) continue;
                            const tag = (node.tagName || '').toLowerCase();
                            // Remove canvas iframes pointing to usercontent.com
                            if (tag === 'iframe') {
                                const src = node.getAttribute('src') || '';
                                if (src.includes('usercontent.com') && (src.includes('immersive') || src.includes('canvas'))) {
                                    console.warn('[Canvas-Blocker] Removing canvas iframe:', src.substring(0, 80));
                                    node.remove();
                                }
                            }
                            // Remove known canvas panel elements
                            const cls = (node.className || '').toString().toLowerCase();
                            if (tag === 'user-facing-canvas' || tag === 'immersive-canvas-panel' ||
                                tag === 'code-block-canvas' ||
                                cls.includes('canvas-container') || cls.includes('immersive-canvas')) {
                                console.warn('[Canvas-Blocker] Removing canvas element:', tag);
                                node.remove();
                            }
                        }
                    }
                ...
                window.__canvasBlockerActive = true;
            }""")
            log("Canvas MutationObserver blocker injected")
        except Exception as e:
            log(f"Canvas MutationObserver injection skipped: {e}")
        ...
        def wait_for_completion():
            ...
            while time.time() - start_wait < max_wait:
                ...
                # 3. Check for Canvas activation during generation
                canvas_check_counter += 1
                if canvas_check_counter % 3 == 0:  # Every ~15s
                    try:
                        for frame in page.frames:
                            frame_url = getattr(frame, 'url', '') or ''
                            if 'usercontent.com' in frame_url and ('immersive' in frame_url or 'canvas' in frame_url):
                                canvas_detected_during_gen = True
                                log(f"  [Canvas] Canvas iframe detected during generation: {frame_url[:80]}")
                                # Force-remove canvas elements from DOM
                                page.evaluate("""() => {
                                    document.querySelectorAll(
                                        'iframe[src*="usercontent"], iframe[src*="immersive"], ' +
                                        'user-facing-canvas, [class*="canvas-panel"], [class*="immersive-canvas"]'
                                    ).forEach(el => el.remove());
                                }""")
                                break
                        if not canvas_detected_during_gen and detect_canvas_active(page):
                            canvas_detected_during_gen = True
                            log("  [Canvas] Canvas detected via DOM during generation")
                    except Exception:
                        pass
        ...
        # --- POST-GENERATION CANVAS CHECK ---
        if canvas_detected_during_gen or detect_canvas_active(page):
            log("  [Canvas] Canvas was active — attempting content recovery...")
            try:
                canvas_text = ""
                # Try extracting text from Canvas iframes
                for frame in page.frames:
                    frame_url = getattr(frame, 'url', '') or ''
                    if 'usercontent.com' in frame_url or 'immersive' in frame_url:
                        try:
                            canvas_text = frame.evaluate("document.body ? document.body.innerText : ''")
                            if canvas_text and len(canvas_text) > 100:
                                log(f"  [Canvas] Recovered {len(canvas_text)} chars from Canvas iframe")
                                break
                        except Exception:
                            pass
                # Try extracting from Canvas DOM elements
                if not canvas_text or len(canvas_text) < 100:
                    canvas_text = page.evaluate("""() => {
                        const selectors = ['user-facing-canvas', '.canvas-container',
                                          '[class*="canvas-panel"]', '[class*="immersive"]',
                                          'code-block-canvas'];
                    ...
                    if canvas_text:
                        log(f"  [Canvas] Recovered {len(canvas_text)} chars from Canvas DOM")
                if canvas_text and len(canvas_text) > 100:
                    canvas_save = out_json.replace(".json", "_canvas.txt")
                    os.makedirs(os.path.dirname(canvas_save), exist_ok=True)
                    with open(canvas_save, 'w', encoding='utf-8') as f:
                        f.write(canvas_text)
                    log(f"  [Canvas] Saved canvas content: {os.path.basename(canvas_save)}")
            except Exception as e:
                log(f"  [Canvas] Canvas recovery failed: {e}")
        ...
        # --- (Canvas content recovered above if Canvas was active) ---

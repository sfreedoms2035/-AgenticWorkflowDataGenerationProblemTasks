...
def repair_metadata(task, filename):
    ...
    for key in REQUIRED_KEYS:
        if key not in task or not task[key] or task[key] == "..." or task[key] == "{pdf_name}":
            if key == "training_data_id": task[key] = f"TD-GPF-{context['doc']}-T{context['turn']}t{context['task']}-{today}-v1.0"
            elif key == "prompt_version": task[key] = "Problems_V1.1"
            ...
            elif key == "task_type": task[key] = "formalized_problem"
            elif key == "affected_role": task[key] = "Systems Engineer"
            elif key == "date_of_generation": task[key] = today
            elif key == "key_words": task[key] = ["Formalization", "SysML", "Python ABC", "Optimization"]
            elif key == "summary": task[key] = f"Complex Problem Formulation for {context['doc']} - Turn {context['turn']}"
            elif key == "difficulty": task[key] = "95"
            elif key == "evaluation_criteria": task[key] = ["Formal Specification Correctness", "SysML Accuracy"]
            fixed = True
    return fixed
...
def repair_structured_answer(task):
    # GPF blocks are too complex to synthesize from raw markdown text accurately.
    # We will defer to full regeneration instead.
    return False
    ...
    for conv in task.get("conversations", []):
        if conv.get("role") == "assistant":
            ...
            for title, prefix in COT_TITLES_TO_NUMBERS.items():
                ...
                # Check if this title already has the correct prefix nearby.
                # Look for patterns like "1. Initial Query" or "1.1. Deconstruct"
                already_prefixed = re.search(
                    rf'(?:^|\n)\s*\**\s*{re.escape(prefix)}\s+{escape_title}',
                    reasoning
                )
                if already_prefixed:
                    continue  # Already correctly numbered, skip
                
                # Also skip if ANY digit prefix precedes this title (e.g., "1. 1. Title" already broken)
                has_any_number = re.search(
                    rf'(?:^|\n)\s*\**\s*\d+\.(?:\d+\.)?\s+{escape_title}',
                    reasoning
                )
                if has_any_number:
                    continue  # Has some numbering already, don't add more
                
                # Safe to add: title exists without any number prefix
                pattern = rf'((?:^|\n)\s*\**\s*)({escape_title})'

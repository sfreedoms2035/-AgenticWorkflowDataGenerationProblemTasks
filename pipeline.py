...
import os
import sys
# Force UTF-8 output on Windows (prevents cp1252 encoding errors with emoji/box chars)
...
def build_generation_prompt(variation, turn, task_idx, doc_name, mode, is_soft_retry=False):
    ...
    Uses block-based output format (!!!!!BLOCK-NAME!!!!!) for robust extraction.
    Gemini outputs 17 labeled text blocks; Python assembles JSON from them.
    """
...
## 1. CORE MISSION
Generate exactly 1 distinct Generalized Problem Formulation (GPF) Task. You will convert the document's implicit engineering pain points into rigorous, machine-interpretable formal mathematical problems. You do not solve problems; you formalize them.
...
*   **Reasoning (CoT) Minimum 10k Chars:** Act as a Concept Engineer actively wrestling with how to translate vague engineering pain-points into pure mathematics. Explicitly draft mapping tables between real-world physics and mathematical sets inside the CoT.
*   **Answer Length (>15k Chars):** You must write at a PhD level.
    *   The `GPF-FORMAL-SPECIFICATION` block MUST include pure mathematical set-theory formalisms, strict objective functions (e.g., `argmin(f(x))`), AND a massive Python abstract base class (ABC) stub acting as the programmatic interface.
    *   The `GPF-ARCHITECTURE` block MUST include 100+ lines of raw XML SysML definitions.
    *   The `GPF-EVALUATION` block MUST include complete, runnable Python script logic capable of benchmarking the problem.
...
The user turn must be written entirely in-character as an engineer facing a real crisis. It must NEVER sound like a prompt engineer commanding an AI.

**RULE 2: THE INTERNAL MONOLOGUE (No Task Generation Thoughts)**
Your reasoning block is the internal brain of the Expert Engineer actively solving the problem. NEVER mention the user's prompt structure, the source document, or the goal of generating tasks.

**RULE 3: THE FOLLOW-UP QUESTIONS (Natural Dialogue)**
The follow-up questions must be natural, organic engineering questions referencing specific components or equations from the GPF answer.
...
## 4. STRICT OUTPUT FORMAT & INTERFACE CONSTRAINTS:
VIRTUAL TERMINAL PERSONA: You are a legacy VT100 Data Terminal. You lack the hardware to render side-panels, UI extensions, or code editors. Any attempt to use "Canvas", "Gems", or side-panels will result in a hardware system crash. All output MUST be a raw text stream in the main chat response window.

You MUST output your response using exactly these block delimiters. Each block starts with !!!!!BLOCK-NAME!!!!! on its own line.
Write ALL code (Python, XML, SysML) as RAW INLINE PLAINTEXT with space-indentation. Do NOT use triple-backtick markdown fenced code blocks (e.g. ```python, ```xml) — they trigger Canvas mode and destroy the environment.

<block_schema>
BLOCKS (17 total — output ALL of them in this exact order):
...
{{
  ...
  "summary": "One-sentence highly technical GPF summary.",
  "difficulty": "{diff}",
  "evaluation_criteria": ["criterion1", "criterion2"]
}}

!!!!!REASONING!!!!!
(The COMPLETE 10,000+ char 8-step CoT monologue. All 31 sub-elements mandatory. Raw text, NOT inside JSON.)

!!!!!TURN-1-USER!!!!!
[Thinking] (Immersive in-character 3-paragraph engineering crisis prompt demanding GPF v2.1 formalization. Each paragraph 3+ sentences.)

!!!!!GPF-HEADER!!!!!
Problem ID: P-<DOMAIN>-<SUBDOMAIN>-<SEQ>
Canonical Name: (full descriptive name)
Problem Version: 2.1
Last Update Date: {date_str}
Estimated Origin Date/Period: (from document)
Primary Domain(s): (hierarchical :: delimited)
Keywords: ["kw1", "kw2", ...]

!!!!!GPF-PROBLEM-DEFINITION!!!!!
(MINIMUM 2000 chars. Must include: 1.1 Problem Statement Narrative >=500 chars, 1.2 Formal Inputs with >=4 typed entries, 1.3 Formal Outputs with >=3 typed entries, 1.4 Task Definition, 1.5 Ontological Scaffolding with >=5 definitions)

!!!!!GPF-ABSTRACTION-LEVEL!!!!!
(MINIMUM 500 chars. 2.1 Description, 2.2 Indicators/Criteria >=3 items, 2.3 Assigned Abstraction Level)

!!!!!GPF-ARCHITECTURE!!!!!
(MINIMUM 3000 chars. 3.1 Architectural Description >=300 chars, 3.2 Formal Architectural Model with >100 LINES raw XML SysML as RAW PLAINTEXT)

!!!!!GPF-FORMAL-SPECIFICATION!!!!!
(MINIMUM 3500 chars. 4.1 Symbolic Representation >=10 set-theory equations, 4.2 Formal Requirements >=5 REQ-IDs, 4.3 Code Representation: MASSIVE Python ABC stub >=80 lines as RAW PLAINTEXT)

!!!!!GPF-EVALUATION!!!!!
(MINIMUM 2500 chars. 5.1 Success Metrics >=5 quantitative, 5.2 Acceptance Criteria, 5.3 Verification Code: complete Python verifier >=60 lines as RAW PLAINTEXT)

!!!!!GPF-CONSTRAINTS!!!!!
(MINIMUM 600 chars. 6.1 Operational Constraints >=5 items, 6.2 Contextual Assumptions >=3 items)

!!!!!GPF-CLASSIFICATION!!!!!
(MINIMUM 500 chars. 7.1 Problem Type, 7.2 Complexity Scoring, 7.3 Usefulness Scoring)

!!!!!GPF-RELATIONAL-NETWORK!!!!!
(MINIMUM 500 chars. 8.1 Related Concepts >=5 items, 8.2 Related Problems >=3 items)

!!!!!GPF-EXEMPLARS!!!!!
(MINIMUM 600 chars. 9.1 Illustrative Input Instance with CONCRETE numeric data, 9.2 Illustrative Solution Instance with CONCRETE output values)

!!!!!GPF-HISTORY!!!!!
(MINIMUM 300 chars. 10.1 Derivation Source, 10.2 Version History)

!!!!!TURN-3-USER!!!!!
[No Thinking] (Technical follow-up question >=100 chars referencing a specific component or equation from the GPF answer)

!!!!!TURN-4-ASSISTANT!!!!!
(Brief 200-500 char highly technical response. MUST NOT start with "[No Thinking]" — that tag is USER-only.)

!!!!!TURN-5-USER!!!!!
[No Thinking] (Second follow-up question >=100 chars about a DIFFERENT component — e.g., adapting Python verification or XML architecture)

!!!!!TURN-6-ASSISTANT!!!!!
(Brief 200-500 char highly technical response. MUST NOT start with "[No Thinking]" — that tag is USER-only.)
</block_schema>

## 5. THE 8-STEP COT MONOLOGUE TEMPLATE (ALL 31 SUB-ELEMENTS MANDATORY)
Populate this exact template inside !!!!!REASONING!!!!!. Do not skip any numbering.

1. Initial Query Analysis & Scoping
1.1. Deconstruct the Request: Detailed analysis of the core ENGINEERING problem. You are solving this problem, not generating it.
...
3.6. Formal Requirements Extraction: Define at least 5 strict requirements with IDs and Pass Criteria.
...
5.1. First Pass Execution: Draft the massive initial algorithmic logic and derivations.
5.2. Deep Analysis & Failure Modes: Generate a 15-row FMEA markdown table.
5.3. Trigger 1 (Verification): Find and fix a critical flaw.
5.4. Trigger 2 (Adversarial): Critique against worst-case SOTIF edge cases.
5.5. Refinement Strategy (Version 2.0): Corrected, hardened, production-ready logic.
6. Comparative Analysis & Synthesis
6.1. Comparison Matrix: 6x5 markdown comparison table.
...
7.2. Detailed Recommended Solution: Plan the exact structure.
7.3. Implementation Caveats & Next Steps: Deployment risks.
...
ANTI-TRUNCATION: Prioritize COMPLETING all 17 blocks over adding more detail. A truncated response missing blocks is worse than a slightly shorter but complete one.
</instructions>"""
    return prompt
...
def build_repair_prompt(validation_report, original_prompt_text):
    ...
    lines = [
        ...
        "CRITICAL: You MUST regenerate the response using the FULL 17-BLOCK SCHEMA (!!!!!METADATA!!!!! through !!!!!TURN-6-ASSISTANT!!!!!).",
...
def run_playwright(pdf_path, prompt_file):
    ...
    try:
        ...
        print(f"  ❌ Playwright timed out after 900s (15 min). Flagging as failure.")
        return False
...
def process_task(pdf_path, doc_short, doc_name, turn, task_idx,
    ...
    while gemini_attempts < MAX_GEMINI_ATTEMPTS:
        ...
        # If there's a fatal overarching error, capture it natively
        if report.get("error"):
            violations.append(f"FATAL VALIDATOR ERROR: {report.get('error')}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n\n  \u26a0\ufe0f Pipeline interrupted by user (Ctrl+C). Progress has been saved.")
        print("  \U0001f4a1 Resume with: python pipeline.py --resume")
        sys.exit(130)
